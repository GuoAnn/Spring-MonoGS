...
class FrontEnd(mp.Process): 
    def __init__(self, config,spring_visualize=False): 
        super().__init__()
        self.gt_pose = os.environ.get("USE_GT_POSE") == "True" 
        self.spring_visualize = spring_visualize
        self.config = config
    ...
    def initialize_spring_mass(self, xyz: torch.Tensor):
        ...
        if xyz.dim() == 2:
            xyz = xyz.unsqueeze(0)  # 形状变为 [1, N, 3]
    ...
    def push_to_gui(self):
        ...
        if self.visualize_springs and self.current_template is not None:
            anchors, connections = self.current_template
                # 显式转到 CPU 并分离计算图
            anchors_cpu = anchors.detach().cpu().numpy()
            connections_cpu = connections.detach().cpu().numpy()
    ...
    def add_new_keyframe(self, cur_frame_idx, depth=None, opacity=None, init=False):
        ...
        if self.monocular:
            ...
            if init:
                ...
                anchor_data = {
                    "kf_id": cur_frame_idx,
                    "anchors": anchors.detach().cpu().numpy(),   # 转换为CPU数据避免共享内存问题
                    "connections": connections.cpu().numpy()
                }
